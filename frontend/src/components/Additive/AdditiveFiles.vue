<template>
    <div><br>
        <div class="text-center mb-4">
            <h1>Gestión de Archivos-Bodega de Aditivos</h1>
        </div><br>
    ...
    <div class="card-body">
        ...
        <div id="transporte" class="card mb-4">
          ...
          <div class="card-body">
            ...
            <ul class="list-group">
                <li v-for="file in paginatedTransporteFiles" :key="file" class="list-group-item d-flex justify-content-between align-items-center">
                    {{ file }}
                    <div class="btn-group">
                        <!-- Botón de descarga -->
  ...
  <li
    v-for="page in totalTransportePages"
    :key="page"
    :class="['page-item', { active: page === currentTransportePage }]"
  >
    <button
      @click="goToPage('transporte', page)"
      class="page-link btn btn-success"
    ...
    Siguiente
        ...
        <div id="producto" class="card mb-4">
          ...
          <div class="card-body">
            ...
            <ul class="list-group">
                <li v-for="file in paginatedProductoFiles" :key="file" class="list-group-item d-flex justify-content-between align-items-center">
                    {{ file }}
                    <div class="btn-group">
                        <!-- Botón de descarga -->
  ...
  <ul class="pagination mb-0">
  <li
    v-for="page in totalProductoPages"
    :key="page"
    :class="['page-item', { active: page === currentProductoPage }]"
  >
    <button
      @click="goToPage('producto', page)"
      class="page-link btn btn-success"
  ...
  export default {
    data() {
      return {
        ...
        supplier: "",
        selectedFile: null,
        providers: ["PROISAL", "QUIMATEC", "DISAROMATI", "GRANOTEC", "DSM", "ARTETA Y MURTHINO-ALIMEC", "MARCSEAL", "CODAN", "ENGRAIN"],
        transporteFilter: "", // Filtro para archivos en Transporte
        productoFilter: "",   // Filtro para archivos en Producto
        products: ["ÁCIDO ASCÓRBICO", "ADITIVO TURMERIC", "PREMEZCLA VITAMÍNICA", "SALSA DE QUESO", "SALSA DE TOMATE", "SALSA DE ESPINACA", "HYDROMAX", "VOLMAX", "ALPHAX"], // Lista de productos
...
async submitFile() {
  ...
  try {
    ...
    const response = await axios.post(
      `${process.env.VUE_APP_API_URL}/api/submit-just-one-file`,
      formData,
      {
        headers: {
  ...
  height: 20px;
  display: block;  /* Hace que el icono ocupe el tamaño completo del contenedor */
  margin: 0 auto;  /* Centra el icono en el botón */
}
...
.list-group-item {
  ...
  flex-wrap: wrap; /* Permite que el texto y los botones bajen de línea si es necesario */
}

/* Controla el texto de los nombres */
.list-group-item > span {
  flex: 1 1 auto; /* Ocupa el espacio disponible */
  min-width: 0; /* Permite que el texto largo se ajuste */
  text-overflow: ellipsis; /* Agrega puntos suspensivos si es muy largo */
  overflow: hidden;
  white-space: nowrap; /* Mantiene todo en una línea si hay espacio */
}

/* Botones para que se alineen correctamente y no se desborden */
.list-group-item .btn-group {
  flex-shrink: 0; /* Evita que los botones cambien de tamaño */
  display: flex;
}
